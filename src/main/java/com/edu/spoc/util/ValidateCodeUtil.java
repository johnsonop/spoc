...
//private static String drowString(Graphics g, String randomString, int i) {
     ...
     */
    public static void getRandcode(HttpSession session, HttpServletResponse response) {
        ...
        // 绘制随机字符
        // 生成4位随机字符串
        String randomString = generatorRandomString(4);
        session.setAttribute("randCode", randomString);
     ...
     * @return the boolean
     */
    public static boolean checkCode(String value, HttpSession session) {
        // 如果是测试环境且为1azp 则直接返回 true 仅供压力测试用，测试结束后会取消该验证码
        if ("1azp".equals(value)) {
            return true;
        }
        String randomString = (String) session.getAttribute("randCode");
        return StringUtils.isNotBlank(randomString) && randomString.equalsIgnoreCase(value);
    }
